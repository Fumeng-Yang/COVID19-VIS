...
my_theme <- theme(
                                      ...
                                      #rect,
                                      #text,
  title = element_text(size = FONT_SIZE_BIG, colour = TEXTCOLOR, face = "bold", family = "Avenir", vjust = 1, hjust = 2), 
                                      #aspect.ratio,
...
first_day <- as.Date("2020-03-21") # to select a dat
...
df_states %>% filter(state == "RI") %>%
    ggplot() %>%
    + geom_label(x = first_day, y = 900, color = "black", label = "death", size = 2, hjust = 0) %>%
    + geom_label(x = first_day, y = 800, color = tableau10[1], label = "positiveIncrease", size = 2, hjust = 0) %>%
    + geom_label(x = first_day, y = 700, color = tableau10[2], label = "hospitalizedCurrently", size = 2, hjust = 0) %>%
...
##  TestPositiveRate against TestedPopulationRate
